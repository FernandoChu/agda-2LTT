...
# Cursed modifications because the path.parents are:
# [PosixPath('"src/foundation-2LTT'), PosixPath('"src'), PosixPath('.')]
# Python adds a `"` if there's unicode for some reason...
def get_subdirectories_recursive(startpath):
    ...
    for path in git_tracked_files:
        if (startpath in path.parents) or (str(path).startswith('"' + str(startpath))):
            relative_path = str(path.parent)[rootlen:]
            if relative_path:
                if relative_path.startswith('/'):
                    subdirectories.add(relative_path[1:])
                else:
